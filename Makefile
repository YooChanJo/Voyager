...
ifeq ($(config),debug)
  GLFW_config = debug
  Glad_config = debug
  Voyager_config = debug

else ifeq ($(config),release)
  GLFW_config = release
  Glad_config = release
  Voyager_config = release

else
  $(error "invalid configuration $(config)")
endif

PROJECTS := GLFW Glad Voyager
...
Voyager: GLFW Glad
ifneq (,$(Voyager_config))
	@echo "==== Building Voyager ($(Voyager_config)) ===="
	@${MAKE} --no-print-directory -C . -f Voyager.make config=$(Voyager_config)
endif

clean:
	@${MAKE} --no-print-directory -C src/vendor/GLFW -f Makefile clean
	@${MAKE} --no-print-directory -C src/vendor/Glad -f Makefile clean
	@${MAKE} --no-print-directory -C . -f Voyager.make clean
